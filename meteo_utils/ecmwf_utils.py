...
ECMWF_ZO_LUT = {
    1: ["Crops, mixed farming", "L", 0.25, 0.25e-2],
...
def download_CDS_data(
    ...
    s = {"variable": variables, "data_format": "grib", "download_format": "unarchived"}
    s["date"] = date_start.strftime("%Y-%m-%d") + "/" + date_end.strftime("%Y-%m-%d")
    if area:
        s["area"] = area
...
def download_ADS_data(
    ...
    s = {"variable": variables, "data_format": "grib"}
    s["date"] = date_start.strftime("%Y-%m-%d") + "/" + date_end.strftime("%Y-%m-%d")
...
def get_ECMWF_data(
    ...
    midnight_local = dt.datetime.combine(date_local, dt.time()).replace(
    ...
    for field in meteo_data_fields:
        if field == "TA":
            ...
            # elevation data
            z = _ECMWFRespampleData(
                z, gt, proj, template_file=elev_file, resample_alg="bilinear"
            )
            t2m = _ECMWFRespampleData(t2m, gt, proj, template_file=elev_file)
            data = calc_air_temperature_blending_height(
                t2m, elev_data + z_bh, z_0=z + 2
            )
            ...
            d2m = calc_dew_temperature_blending_height(d2m, elev_data, z_0=z + 2.0)
            data = calc_vapour_pressure(d2m)

        elif field == "WS":
            if "u100" in xds.variables and "v100" in xds.variables:
                u100, gt, proj = _getECMWFTempInterpData(
                    xds, "u100", beforeI, afterI, frac
                )
                v100, gt, proj = _getECMWFTempInterpData(
                    xds, "v100", beforeI, afterI, frac
                )
                # Combine the two components of wind speed and calculate speed at blending height
                ws = calc_wind_speed(u100, v100)
                z_u = 100
            else:
                u10, gt, proj = _getECMWFTempInterpData(
                    xds, "u10", beforeI, afterI, frac
                )
                v10, gt, proj = _getECMWFTempInterpData(
                    xds, "v10", beforeI, afterI, frac
                )
                # Combine the two components of wind speed and calculate speed at blending height
            ...
            if z_bh == z_u:
                ...
                if "sr" in xds.variables:
                    z_0M, gt, proj = _getECMWFTempInterpData(
                        ecmwf_data_file, "sr", beforeI, afterI, frac
                    )
                    z_0M = _ECMWFRespampleData(z_0M, gt, proj, template_file=elev_file)
                elif "fsr" in xds.variables:
                    z_0M, gt, proj = _getECMWFTempInterpData(
                        ecmwf_data_file, "fsr", beforeI, afterI, frac
                    )
                    z_0M = _ECMWFRespampleData(z_0M, gt, proj, template_file=elev_file)
            ...
            if is_forecast:
                ...
                ref_hour = -1
            data, gt, proj = _get_ECMWF_cummulative_data(xds, "tp", ref_hour, after)
            data = data * 1000
            data = _ECMWFRespampleData(data, gt, proj, template_file=elev_file)
            ...
            if is_forecast:
                ...
                ref_hour = -1
            data, gt, proj = _get_ECMWF_cummulative_data(xds, "strd", ref_hour, after)
            ...
            if "z" in xds.variables:
                ...
                z = outDs.GetRasterBand(1).ReadAsArray()
                z = _ECMWFRespampleData(
                    z, gt, proj, template_file=elev_file, resampleAlg="bilinear"
                )
                del outDs
            ...
            eb, ed = solar.calc_global_horizontal_radiance_clear_sky(
                doy, sza, aot550, tcwv, sp, t, altitude=elev_data, calc_diffuse=True
            )

            saa = _ECMWFRespampleData(saa, gt, proj, elev_file, resample_alg="bilinear")
            non_shaded = du.non_occluded_terrain(elev_file, saa, sza, smooth_radius=1)
            slope = gu.raster_data(slope_file)
            slope[slope < 0] = np.nan
            aspect = gu.raster_data(aspect_file)
            aspect[aspect < 0] = np.nan
            lat_hr = _ECMWFRespampleData(
                lats, gt, proj, elev_file, resample_alg="bilinear"
            )
            lon_hr = _ECMWFRespampleData(
                lons, gt, proj, elev_file, resample_alg="bilinear"
            )
            ...
            data = calc_radiance_tilted_surface_op(
                eb,
                ed,
                doy,
                ftime,
            ...
            if aod550_data_file is not None:
                ...
                aod550, gt, proj = _getECMWFTempInterpData(
                    xds, "aod550", beforeI, afterI, frac
                )
            ...
            data, gt, proj = _getECMWFSolarData(
                xds,
                midnight_UTC,
                elev_file,
                slope_file,
            ...
            data, gt, proj = _get_cummulative_data(
                ecmwf_data_file,
                "tp",
                midnight_UTC,
                elev_file,
            ...
            data = mean_temperature(
                xds, elev_file, timedate_UTC, time_zone=time_zone, z_t=2
            )
...
def calc_solar_irradiance(
    ...
    sdn = solar.calc_global_horizontal_radiance_clear_sky(
        doy, sza, aot, tcwv, p, ta, altitude=elev
    )
    return sdn
...
def _ECMWFRespampleData(data, gt, proj, template_file, resample_alg="cubicspline"):
    ...
    ds_out_proj = gu.resample_with_gdalwarp(
        ds_out, template_file, resample_alg=resample_alg
    )
...
def _get_cummulative_data(
    xds, var_name, date_time, elev_file, time_window=24, is_forecast=True
):
    ...
    date_1, _, _ = _bracketing_dates(dates, date_time + dt.timedelta(hours=time_window))
...
def _getECMWFSolarData(
    ...
    for date_i in range(date_0 + 1, date_1 + 1):
        ...
        if np.any(sza <= SZA_THRESHOLD_CORRECTION):
            ...
            if "aod550" in xds.variables:
                ...
                extent = [gt[0], gt[3] + gt[5] * y_size, gt[0] + gt[1] * x_size, gt[3]]
                aot550 = gdal.Warp(
                    "",
                    tempds,
                    format="MEM",
    ...
    cummulated_value /= time_window * 3600.0
    return cummulated_value, gt, proj
...
def daily_reference_et(
    ...
    if not date_0:
        date_0 = 0
    # Get the time right before the temporal witndow set
    date_1, _, _ = _bracketing_dates(dates, midnight_UTC + dt.timedelta(hours=24))
    ...
    t_max = np.full(elev_data.shape, -99999.0)
    t_min = np.full(elev_data.shape, 99999.0)
    p_array = []
    u_array = []
    ea_array = []
    es_array = []
    for date_i in range(date_0 + 1, date_1 + 1):
        ...
        t_air, td, u, v, p = map(np.ma.filled, [t_air, td, u, v, p], 5 * [np.nan])
        ea = td.copy()
        ...
        t_air = calc_air_temperature_blending_height(
            t_air, elev_data + z_t, z_0=z + 2.0
        )
    ...
    sdn_mean, _, _ = _getECMWFSolarData(
        xds,
        midnight_UTC,
        elev_file,
        slope_file,
    ...
    lats = _ECMWFRespampleData(
        lats, gt, proj, template_file=elev_file, resample_alg="bilinear"
    )
    doy = float(timedate_UTC.strftime("%j"))
...
def mean_temperature(xds, elev_file, timedate_UTC, time_zone=0, z_t=2):
    if "valid_time" in xds.variables:
        dates = xds["valid_time"].values
    else:
        dates = xds["time"].values
...
def correct_solar_irradiance(
    ...
    sdn_cs = solar.calc_global_horizontal_radiance_clear_sky(
        doy, sza, aot550, tcwv, press, t, altitude=z_0, calc_diffuse=False
    )
    sdn_cs = np.maximum(sdn_cs, 0.0)
    # Cloudiness factor
    fci_cl = np.clip(sdn / sdn_cs, 0.0, 1)
    ...
    if np.any(valid):
        ...
        if illum_f is not None and saa is not None and lon is not None:
            ...
            skyl = np.clip(1.0 - fci_cl[valid] * beam_ratio, 0, 1)
            ...
            bdn, ddn = calc_radiance_tilted_surface_op(
                bdn,
                ddn,
                doy,
                ftime,
...
def calc_radiance_tilted_surface_op(
    bdn, ddn, doy, ftime, lon, sza, f1, f2, f3, svf=1, non_shaded=None
):
...
def incidence_angle_tilted_optimized(doy, ftime, lon, f1, f2, f3, stdlon=0):
    ...
    cos_theta_i = (
        sin_delta * f1 + cos_delta * cos_omega * f2 + cos_delta * sin_omega * f3
    )
...
def iterception_bastiaansen(lai, rain, f_c=1, specific_storage=0.2):
    ...
    i_mm = max_storage * (1.0 - 1.0 / (1.0 + f_c * rain / max_storage))
...
def iterception_calder(lai, rain, f_c=1, specific_storage=0.2, q=0.5):
    ...
    i_mm = max_storage * (1.0 - np.exp(-q * f_c * rain / max_storage))
