...
def get_forecast_data(ecmwf_data_file,
                      timedate_UTC,
                      meteo_data_fields,
                      elev_file,
                      z_bh,
    ...
    for field in meteo_data_fields:
        if field == "TA":
            ...
            z, *_ = _getECMWFTempInterpData(xds, "z",
                                            beforeI, afterI, frac)
            z /= GRAVITY

            sp, *_ = _getECMWFTempInterpData(xds, "sp",
                                             beforeI, afterI, frac)
            ...
            ws10 = calc_wind_speed(u10, v10)
            if z_bh == 10:
                data = _ECMWFRespampleData(ws10, gt, proj, template_file=elev_file)
            else:
                ws10 = _ECMWFRespampleData(ws10, gt, proj, template_file=elev_file)
                # z_0M, gt, proj = _getECMWFTempInterpData(xds, "fsr",
                #                                          beforeI, afterI,
                #                                          frac)
                # z_0M = _ECMWFRespampleData(z_0M, gt, proj, template_file=elev_file)
                # Calculate wind speed at blending height.
                z_0M = np.full_like(ws10, 0.25)
                data = calc_windspeed_blending_height(ws10, z_0M, z_bh)
            ...
            data, gt, proj = _getECMWFTempInterpData(xds,
                                                     "tp",
                                                     beforeI, afterI, frac)
            # Convert to mm
            data = data * 1000
            ...
            data, gt, proj = _getECMWFTempInterpData(xds,"strd",
                                                     beforeI, afterI, frac)
            ...
            tcwv, *_ = _getECMWFTempInterpData(xds,"tcwv",
                                              beforeI, afterI, frac)
            tcwv = calc_tcwv_cm(tcwv)
            tcwv = _ECMWFRespampleData(tcwv, gt, proj, elev_file)
            aot550, *_ = _getECMWFTempInterpData(xds,"aod550",
                                                 beforeI, afterI, frac)
            ...
            aod550, gt, proj = _getECMWFTempInterpData(xds,"aod550",
                                                       beforeI, afterI, frac)
...
def _get_gdal_params(xds, var_name):
    proj = xds[var_name].spatial_ref.crs_wkt
    xds["t2m"].rio.transform()
    gt = xds.rio.transform()
    gt = [gt[2], gt[0], gt[1], gt[5], gt[3], gt[4]]
    return gt, proj


def _getECMWFTempInterpData(xds, var_name, before, after, f_time):
    # Get some metadata
    gt, proj = _get_gdal_params(xds, var_name)
    data_before = xds[var_name][:, :, before]
    data_after = xds[var_name][:, :, after]
...
def correct_solar_irradiance(doy,
    ...
    if aot550 is None:
        aot550 = np.full(sza.shape, RURAL_AOT_25KM)
    if tcwv is None:
        tcwv = np.full(sza.shape, TCWV_MIDLATITUDE_SUMMER)
